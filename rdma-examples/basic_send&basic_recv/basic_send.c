...
#define WELL_KNOWN_QKEY 0x11111111
	...
	qp_modify_attr.port_num        = dev_port;			//dev_port:
	...
	/*
	 * Once the QP is transistioned into the RTS state, the QP begins send processing 
	 * and is fully optional. The user may post send requests with ibv_post_send
	 */
	...
	/*
	 *@description: create an address handle associated with the pd, An AH contains all of the necessary data
	 *to reach a remote destination. In connected transport modes(RC, UC), the AH is associated with a QP. 
	 *In the datagram transport modes, the AH is associated with a work request(WR)
	 */
...
#define TEXT_MSG "Hello UD :)"
	...
	/*
	 *@description: ibv_post_send posts a linked list of WRs to a QP's send queue.
     *This operation is used to initiate all communication, including RDMA operations.
	 *Processing of the WR list is stopped on the first error and a pointer to the
	 *offending WR is returned in bad_wr
	 */
