...
struct nvme_rdma_device {				//nvme over rdma device struct, add by dujiong
...
struct nvme_rdma_request {				//nvme over rdma request, add by dujiong
...
/*GET Work Queue Element from capsule info from sge, and push it to RDMA send queue*/  //add by dujiong
		...
		struct ib_send_wr *first, bool flush)
{
	...
	ret = ib_post_send(queue->qp, first, &bad_wr);		//core function
	if (ret) {
...
static int __init nvme_rdma_init_module(void)
{
	...
	/* register nvmf_transport(ie. nvme over rdma) to kernel list*/		//added by dujiong
